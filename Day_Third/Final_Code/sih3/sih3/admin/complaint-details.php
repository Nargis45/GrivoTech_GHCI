...
<a href="../user/assets/images/<?php echo htmlentities($row['proof']);?>" target="_blank"/> View File</a>
<?php } ?></td>
...
<?php $retf=mysqli_query($db,"select univpush.reason as reason,univpush.status as sstatus,univpush.pushDate as rdate from univpush join complain on complain.complaintNumber=univpush.complaintnumber where univpush.complaintnumber='".$_GET['cid']."'");
while($rwf=mysqli_fetch_array($retf))
{
?>
<tr>
<td><b>Reason (From University)</b></td>
<td colspan="5"><?php echo  htmlentities($rwf['reason']); ?> <br><br><b>Remark Date :</b><?php echo  htmlentities($rwf['rdate']); ?></td>
</tr>
...
<?php $rets=mysqli_query($db,"select univsolution.reason as reason,univsolution.pushdate as rdate from univsolution join complain on complain.complaintNumber=univsolution.complaintnumber where univsolution.complaintnumber='".$_GET['cid']."'");
while($rws=mysqli_fetch_array($rets))
{
?>
<tr>
<td><b>Solution (to University)</b></td>
...
<?php }?>
                                          ...
                                          <tr>
											...
											<?php } else{ ?>
                                             ...
                                             <button type="button" class="btn btn-primary">Reply To University</button></a>
											 </td>      
										</tr>
